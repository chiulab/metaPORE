#!/bin/bash
#
#	metapore_preprocess_ncores.sh
#
#	Chiu Laboratory
#	University of California, San Francisco
#
# Copyright (C) 2015 Charles Chiu and Scot Federman - All Rights Reserved
...
if [ $# != 13 ]; then
	echo "Usage: $scriptname <R1 FASTQ file> <S/I quality> <Y/N uniq> <length cutoff; 0 for no length cutoff> <# of cores> <free cache memory cutoff in GB> <Y/N keep short reads> <adapter_set> <start_nt> <crop_length> <temporary_files_directory> <quality_cutoff> <prefix>"
	exit
fi
...
if [[ "$FASTQperCore" -eq 0 ]]
then
	echo -e "$(date)\t$scriptname\tFASTQperCore is 0 - adjusting to FASTQentries"
	FASTQperCore="$FASTQentries"
	cores=1
	LinesPerCore="$numlines"
fi
...
do
	mv $f $f.fastq
	echo -e "$(date)\t$scriptname\tmetapore_preprocess.sh $f.fastq $quality N $length_cutoff $keep_short_reads $adapter_set $start_nt $crop_length $temporary_files_directory >& $f.preprocess.log &"
	metapore_preprocess.sh $f.fastq $quality N $length_cutoff $keep_short_reads $adapter_set $start_nt $crop_length $temporary_files_directory $quality_cutoff >& $f.preprocess.log &
done
